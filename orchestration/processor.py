...
from bucket.po_bucket import get_po_state, update_status, append_thread, compute_flags
...
from g_sheets.sheets_handler import update_po_expected_delivery, update_po_partial_delivery_dates
...
def _extract_any_date(subject: str, body: str) -> Optional[datetime]:
    text = f"{subject}\n{body}".lower()
...
def _infer_accepted_date_from_threads(state: Dict[str, Any]) -> Optional[str]:
    ...
    for msg in reversed(threads):
        ...
        if any((l or "").lower() == "intent:partial_availability" for l in labels) or any("partial" in (l or "").lower() for l in labels):
            dt = _extract_any_date(msg.get("subject") or "", msg.get("body") or "")
            if dt:
...
def _infer_remaining_date_from_threads(state: Dict[str, Any]) -> Optional[str]:
    threads = state.get("threads") or []
...
def _extract_all_dates(subject: str, body: str) -> List[datetime]:
    text = f"{subject}\n{body}".lower()
    out: List[datetime] = []
...
def _extract_promised_delivery_date(subject: str, body: str) -> Optional[datetime]:
    text = f"{subject}\n{body}".lower()
    ...
    return _extract_any_date(subject, body)
...
def process_inbound_email(
    ...
    """
    email_info = classify_email(from_sender, subject, body)
    ...
    if email_info.get("role") == "supplier" and prior_flags.get("delivery_date_past") is True:
        promised = _extract_promised_delivery_date(subject, body)
        if promised:
    ...
    if email_info.get("role") == "supplier" and "partial_availability" in (email_info.get("intents") or []):
        ...
        if not ad_str and not rd_str:
            dates = _extract_all_dates(subject, body)
            if len(dates) == 1:
                ad_dt = dates[0]
            elif len(dates) >= 2:
                ad_dt = dates[0]
    ...
    if email_info.get("role") == "internal" and (state.get("status") or "").upper() == "PARTIAL_AVAILABILITY":
        ...
        if buyer_partial_decision:
            ...
            if not email_info.get("accepted_delivery_date"):
                inferred2 = _infer_accepted_date_from_threads(state)
                if inferred2:
                    email_info["accepted_delivery_date"] = inferred2
            if not email_info.get("remaining_delivery_date"):
                inferred = _infer_remaining_date_from_threads(state)
                if inferred:
    ...
    if promised:
        actions = ([
            {"action": "send_email", "to": "buyer", "scenario": "delivery_date_update_buyer"},
            {"action": "update_status", "value": "DELIVERY_DATE_UPDATED"},
        ] + actions)

    # If supplier reported partial availability, ensure we:
    # 1) ask buyer for decision (including remaining ETA if present)
    # 2) if remaining ETA missing, ask supplier for committed date for remaining items
    # Also dedupe against the existing partial availability rule.
    if partial_flow:
        deduped: List[Dict[str, Any]] = []
        for act in actions:
            if act.get("action") == "send_email" and act.get("scenario") == "partial_quantity_confirmation":
                continue
            if act.get("action") == "update_status" and act.get("value") == "PARTIAL_AVAILABILITY":
                continue
            deduped.append(act)
        actions = deduped
        ...
        if not partial_remaining_dt:
            injected.insert(0, {"action": "send_email", "to": "supplier", "scenario": "partial_availability_request_remaining_date"})
    ...
    if buyer_partial_decision:
        ...
        injected2: List[Dict[str, Any]] = [
            {"action": "send_email", "to": "supplier", "scenario": "partial_availability_buyer_decision_supplier"},
        ]
        if new_status:
            injected2.append({"action": "update_status", "value": new_status})
        actions = injected2 + actions
    ...
    for act in actions:
        if act.get("action") == "update_status":
            ...
            # Unknown action types in POC: ignore safely
